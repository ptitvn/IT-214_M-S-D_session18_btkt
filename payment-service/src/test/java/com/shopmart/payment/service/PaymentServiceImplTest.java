...
class PaymentServiceImplTest {
    ...
    @Test
    void refund_shouldSetRefundedStatus_whenPaymentWasSuccess() {
        Payment payment = Payment.builder()
                .id(1L).orderId(1L).amount(new BigDecimal("25000000")).status(PaymentStatus.SUCCESS).build();
    ...
    @Test
    void refund_shouldThrow_whenPaymentWasNotSuccess() {
        Payment payment = Payment.builder()
                .id(1L).orderId(1L).amount(new BigDecimal("25000000")).status(PaymentStatus.FAILED).build();
        when(paymentRepository.findByOrderId(1L)).thenReturn(Optional.of(payment));

        org.junit.jupiter.api.Assertions.assertThrows(IllegalStateException.class,
                () -> serviceWith(false).refund(1L));
    }
}
