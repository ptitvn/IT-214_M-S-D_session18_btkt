# ShopMart – Đã hoàn thiện đầy đủ Câu 1 → Câu 5 (Saga Pattern + Spring Cloud)

> **Ghi chú của người thực hiện:** toàn bộ các mục `TODO Câu x` trong base project đã được cài đặt
> (Config Server, Eureka, API Gateway, FeignClient + Resilience4j Circuit Breaker, Kafka Choreography
> Saga + compensating rollback, consumer reactive WebFlux, Redis Cache-Aside, unit test rollback/refund).
> Chi tiết thiết kế nằm trong file `GIAI-PHAP.md` ở thư mục gốc — đọc file đó trước khi chạy.
