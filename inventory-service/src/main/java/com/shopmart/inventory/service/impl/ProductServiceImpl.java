...
/**
 * Câu 4: Cache-Aside với Spring Cache + Redis.
 * Cache name "products", key = id sản phẩm.
 * - @Cacheable  : getProductById  -> chỉ truy vấn DB lần đầu, các lần sau lấy từ Redis.
 * - @CachePut   : update/decrease/increase -> ghi đè cache bằng dữ liệu mới nhất (không cần đọc lại DB).
 * - @CacheEvict : delete -> xoá cache vì sản phẩm không còn tồn tại.
 */
@Slf4j
@Service
